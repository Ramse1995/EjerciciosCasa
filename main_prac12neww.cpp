//Semestre 2019 - 2
//Alumno (s):Saldivar Juarez Ramses
//Accion de Animacion comienza con la letra u										
...
void animacion()
{
	...
	if (circuito_inverso) {
		...
		if (recorrido1_inv)
		{
			...
			if (movKitZ < -155)
			{
				recorrido1_inv = false;
				recorrido4_inv = true;
				//se cambia por el recorrido 4
			}
		}

		if (recorrido4_inv)
		{
			rotKit = 90; //cambiar rotacion
			movKitX++;
			if (movKitX > 125)
			{
				recorrido4_inv = false;
				recorrido6_inv = true; //se cambia por el 3 (era el 5)
			}
		}
...
void keyboard(unsigned char key, int x, int y)  // Create Keyboard Function
{
	...
	case 'u':		//  
	case 'U':
		circuito_inverso ^= true; //Activamos/desactivamos la animacíon
		g_fanimacion = false;
		break;
	case 'k':		//
	case 'K':
