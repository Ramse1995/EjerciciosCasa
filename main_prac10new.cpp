...
float movm4 = 0.0;
float movst = 0.0;
float movrotatef = 0.0;
float movllanta = 0.0;
bool g_fanimacion = false;
bool g_fanimacions = true;
...
void InitGL ( GLvoid )     // Inicializamos parametros
{
	...
	kit.VertexNormals();
	sol._3dsLoad("sol.3ds");
	sol.VertexNormals();
	st._3dsLoad("St.3ds");
	st.VertexNormals();
	llanta._3dsLoad("k_rueda.3ds");
	m4._3dsLoad("Wolf.3ds");
	m4.VertexNormals();
	...
	glPushMatrix();
		...
		glPushMatrix();		
			...
			glPushMatrix();
				...
				glRotatef(movKitt, 0, 1, 0);

				sol.GLrender(NULL, _SHADED, 1.0);
				glScalef(2.085, 2.085, 2.085);
				glTranslatef(-100, 0,0);
				glRotatef(movKitt, 0, 1, 0);
				//st.GLrender(NULL, _SHADED, 1.0);
				glTranslatef(0, -3, -1);
...
void animacion()
{
	...
	if (g_fanimacion && g_fanimacions == true)
	{
		movKit += 1.0;
		movKitt += 1.3;
	...
	if (g_fanimacion && g_fanimacions == false)
	{
